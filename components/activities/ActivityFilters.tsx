...
import { ActivityFilter, ActivitySortBy, ActivitySortOrder, Activity } from '../../types/activity';
import { ACTIVITY_CATEGORIES, ACTIVITY_TYPES, ACTIVITY_STATUSES } from '../../data/activityCategories';
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-4">
        ...
        <div>
          ...
          <select
            id="type"
            value={filter.type || ''}
            onChange={(e) => onFilterChange({ ...filter, type: (e.target.value || undefined) as Activity['type'] | undefined })}
        ...
        <div>
          ...
          <select
            id="status"
            value={filter.status || ''}
            onChange={(e) => onFilterChange({ ...filter, status: (e.target.value || undefined) as Activity['status'] | undefined })}
            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          >
        ...
        <div>
          ...
          <div className="flex gap-2">
            <select
              id="sort"
              value={sortBy}
              onChange={(e) => onSortChange(e.target.value as ActivitySortBy, sortOrder)}
      ...
      <div className="mt-4 flex items-center gap-2 flex-wrap">
        ...
        {(filter.search || filter.type || filter.category || filter.status || filter.isFeatured || filter.isPublic || filter.dateFrom || filter.dateTo) && (
          <button
