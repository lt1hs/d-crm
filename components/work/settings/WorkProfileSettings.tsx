...
const WorkProfileSettings: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update local state immediately (no need to refetch from database)
      updateUserProfile({
        fullName: formData.fullName,
        email: formData.email,
        username: formData.username,
        avatar: formData.avatar || '/imgs/default-avatar.png',
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Avatar URL */}
      <div>
        <label htmlFor="avatar-url" className="block text-sm font-medium mb-2">Avatar URL (Optional)</label>
        <input
          id="avatar-url"
          type="url"
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
          Lowercase letters, numbers, and underscores only. Used for @mentions in chat.
        </p>
      </div>
